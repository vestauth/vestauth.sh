...
helpers do
  def handle_download(os, name, arch, version = nil)
    ...
    registry_url = "https://registry.npmjs.org/@vestauth/#{repo}/-/#{filename}"
...
get "/stats/curl" do
  ...
  handle_stats([
    ...
    "@vestauth/vestauth-windows-amd64",
...
get "/darwin/*" do
  ...
  handle_download("darwin", "vestauth", arch, params["version"])
end

get "/linux/*" do
  arch = params["splat"].first
  handle_download("linux", "vestauth", arch, params["version"])
end

get "/windows/*" do
  arch = params["splat"].first
  handle_download("windows", "vestauth", arch, params["version"])
end
